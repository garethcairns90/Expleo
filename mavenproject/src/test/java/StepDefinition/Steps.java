...
public class Steps 
{
	...
	public void user_is_on_the_landing_page_2() throws Throwable
	{
		// Landing page is loaded in setup Function
	}
	...
	{
		Assert.assertTrue(this.landingPage.getURL().contentEquals(Config.gitHubURL));
	...
	public void user_is_on_the_landing_page_7() {
		// Browser is opened in setup Function
	}
	...
	public void user_is_on_the_landing_page_8() {
		// Browser is opened in setup Function
	}
	
	@When("the user searches for the word 'Adipisci7'")
	public void the_user_searches_for_word() throws Throwable {
		WebDriverWait wait = new WebDriverWait(driver, 10);
		wait.until(ExpectedConditions.visibilityOfElementLocated(landingPage.table));
	}
	
	@Then("the content exists on the table")
	public void content_exists_table() throws Throwable
	{		
		boolean found = this.landingPage.searchTableContent("Adipisci7");
	...
	public void user_is_on_the_landing_page_9() {
		// Browser is opened in setup Function
	}
	
	@When("the user clicks on the red button")
	public void the_user_clicks_on_the_red_button() throws Throwable {
	
		WebDriverWait wait = new WebDriverWait(driver, 10);
		wait.until(ExpectedConditions.visibilityOfElementLocated(landingPage.redBtn));
		
		WebElement redBtn = landingPage.getRedBtn();
		this.landingPage.redButtonId = redBtn.getAttribute("id");
		this.landingPage.clickBtn(redBtn);
	}
	...
	{	
		...
		// element id changes whenever the page is refreshed
		Assert.assertFalse(this.landingPage.redButtonId.contentEquals(landingPage.getRedBtn().getAttribute("id")));
	}
	
	// end of scenario 9 ******************************************************************  

	@Given("^user 10 is on the landing page$")
	public void user_is_on_the_landing_page_10() {
		// Browser is opened in setup Function
	}
	
	@When("the user clicks on the green button")
	public void the_user_clicks_on_the_green_button() throws Throwable {
	
		WebDriverWait wait = new WebDriverWait(driver, 10);
		wait.until(ExpectedConditions.visibilityOfElementLocated(landingPage.greenBtn));
		
		WebElement greenBtn = landingPage.getGreenBtn();
		this.landingPage.greenButtonId = greenBtn.getAttribute("id");
		this.landingPage.clickBtn(greenBtn);
	}
	
	@Then("the page is refreshed")
	public void the_page_refreshes_2() throws Throwable
	{	
		WebDriverWait wait = new WebDriverWait(driver, 10);
		wait.until(ExpectedConditions.visibilityOfElementLocated(landingPage.greenBtn));
		
		// element id changes whenever the page is refreshed
		Assert.assertFalse(this.landingPage.greenButtonId.contentEquals(landingPage.getGreenBtn().getAttribute("id")));
	}
